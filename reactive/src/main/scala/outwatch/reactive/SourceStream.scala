...
object SourceStream {
  ...
  def failed[S[_]: Source, A](source: S[A]): SourceStream[Throwable] = new SourceStream[Throwable] {
    def subscribe[G[_]: Sink](sink: G[_ >: Throwable]): Subscription =
      Source[S].subscribe(SinkObserver.create(_ => (), Sink[G].onNext(sink)(_)))
  }
  ...
  @inline def prependSync[S[_]: Source, A, F[_] : RunSyncEffect](source: S[A])(value: F[A]): SourceStream[A] = concatSync(value, source)
  @inline def prependAsync[S[_]: Source, A, F[_] : Effect](source: S[A])(value: F[A]): SourceStream[A] = concatAsync(value, source)
  @inline def prependFuture[S[_]: Source, A](source: S[A])(value: Future[A]): SourceStream[A] = concatFuture(value, source)
  ...
  @inline implicit class Operations[A](val source: SourceStream[A]) extends AnyVal {
    ...
    @inline def prependFuture[A](value: Future[A]): SourceStream[A] = SourceStream.prependFuture(source)(value)
