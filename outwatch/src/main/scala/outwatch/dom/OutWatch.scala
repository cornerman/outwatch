package outwatch.dom

import cats.effect.SyncIO
import monix.execution.Scheduler
import org.scalajs.dom
import org.scalajs.dom._
import outwatch.dom.helpers.SnabbdomOps
import outwatch.util.Store
import snabbdom.{VNodeProxy, patch}

object OutWatch {

  def toSnabbdom(vNode: VNode)(implicit s: Scheduler): SyncIO[VNodeProxy] = SyncIO {
    SnabbdomOps.toSnabbdom(vNode)
  }

  def renderInto(element: dom.Element, vNode: VNode)(implicit s: Scheduler): SyncIO[Unit] = for {
    node <- toSnabbdom(vNode)
    _ <- SyncIO {
      val elem = dom.document.createElement("div")
      element.appendChild(elem)
      patch(elem, node)
    }
  } yield ()

  def renderReplace(element: dom.Element, vNode: VNode)(implicit s: Scheduler): SyncIO[Unit] = for {
    node <- toSnabbdom(vNode)
    _ <- SyncIO {
      val elementNode = VNodeProxy.fromElement(element)
      patch(elementNode, node)
    }
  } yield ()

  def renderInto(querySelector: String, vNode: VNode)(implicit s: Scheduler): SyncIO[Unit] =
    renderInto(document.querySelector(querySelector), vNode)

  def renderReplace(querySelector: String, vNode: VNode)(implicit s: Scheduler): SyncIO[Unit] =
    renderReplace(document.querySelector(querySelector), vNode)

  @deprecated("Use renderInto instead (or renderReplace)", "0.11.0")
  def render(querySelector: String, vNode: VNode)(implicit s: Scheduler): SyncIO[Unit] = renderInto(querySelector, vNode)

  def renderWithStore[S, A](
    initialState: S, reducer: Store.Reducer[S, A], querySelector: String, root: VNode
  )(implicit s: Scheduler): SyncIO[Unit] =
    Store.renderWithStore(initialState, reducer, querySelector, root)
}
